...
object A2lParser {
     ...
     * Comments are stripped first so a commented-out `/end …` or `/begin …`
     * can't corrupt block matching on real-world A2L files.
     */
    fun parse(content: String): A2lParseResult {
        val clean          = stripComments(content)
        val methods        = parseCompuMethods(clean)
        val measurements   = parseMeasurements(clean)
        val recordLayouts  = parseRecordLayouts(clean)
        val axisPts        = parseAxisPts(clean)
        val characteristics = parseCharacteristics(clean)
        return A2lParseResult(methods, measurements, characteristics, axisPts, recordLayouts)
    }

    /**
     * Remove ASAP2 block (`/* … */`) and line (`//`) comments, preserving any
     * that appear inside quoted strings (A2L LongIdentifiers). Comment bodies are
     * replaced with a space so surrounding tokens stay separated.
     */
    internal fun stripComments(content: String): String {
        ...
        while (i < n) {
            ...
            if (inStr) {
                out.append(c)
                if (c == '\\' && i + 1 < n) { out.append(content[i + 1]); i += 2; continue }
                if (c == '"') inStr = false
                i++
            } else when {
     ...
     *
     * LIMITATION (MVP): genuinely quadratic RAT_FUNC, FORM/formula, and TAB_INTP
     * COMPU_METHODs are not supported and fall through to identity below — the map
     * then shows RAW values (unscaled). This is intentional for the current A2L→XDF
     * scope; such methods are uncommon for the calibration tables MxT targets.
     */

...
class Me7LoggerProcess : LoggerManager {
    ...
    private var tempCfgFile: File? = null

    /** Expose current config for testing. */
    internal fun currentConfig(): LoggerConfig? = config
    ...
    override suspend fun connect(config: LoggerConfig) {
        ...
        // Generate temp .cfg from selected variables if no .cfg provided
        val effectiveCfgFile = if (config.cfgFile.isNotEmpty()) {
            val f = File(config.cfgFile)
            if (!f.exists()) {
                _status.value = LoggerStatus.ERROR
            ...
            f
        } else if (config.selectedVariableNames.isNotEmpty()) {
            val tempFile = File.createTempFile("mxt_log_", ".cfg")
            tempFile.deleteOnExit()
            val triples = config.selectedVariableNames.map { Triple(it, "", "") }
            EcuWriter.writeCfgFile(
                outputFile = tempFile,
                ecuFilename = ecuFile.name,
                variables = triples,
                samplesPerSecond = if (config.overrideSamplesPerSecond) config.samplesPerSecond else 20,
                description = "Temporary log configuration generated by MxT"
            )
            tempCfgFile = tempFile
            tempFile
        } else {
            _status.value = LoggerStatus.ERROR
            _statusMessage.value = "No .cfg file or variables selected"
            return
        }

        // Update config with effective .cfg path for buildCommand
        this.config = config.copy(cfgFile = effectiveCfgFile.absolutePath)

        _status.value = LoggerStatus.CONNECTED
        _statusMessage.value = "Ready. ECU: ${ecuFile.name}, Config: ${effectiveCfgFile.name}"
    }

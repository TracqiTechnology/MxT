...
class AxisRescalerComposeE2ETest {
    ...
    @Test
    fun editingOutputYAxisAndRescalingRendersExactInterpolatedValues() = runComposeUiTest {
        setContent {
            AxisRescalerScreen(
                preloadedMap = Map3d(
                    xAxis = arrayOf(10.0, 20.0, 30.0),
        ...
        val outputMiddleY = onNodeWithTag("axis-output-y_cell_0_1")
        outputMiddleY.performClick()
        outputMiddleY.performClick()
        val activeEditor = onNode(hasSetTextAction() and isFocused())
        activeEditor.performTextReplacement("1500")
        activeEditor.performKeyInput { pressKey(Key.Enter) }
        waitForIdle()
        ...
        onNodeWithTag("axis-output-map-cell-1-0").assertTextEquals("2.5")
        onNodeWithTag("axis-output-map-cell-1-1").assertTextEquals("3.5")
        onNodeWithTag("axis-output-map-cell-1-2").assertTextEquals("4.5")
    }
}
